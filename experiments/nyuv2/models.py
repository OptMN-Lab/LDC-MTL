...
class _FairSegNet(nn.Module):
    """SegNet MTAN"""

    def __init__(self):
        super(_FairSegNet, self).__init__()
        # initialise network parameters
        filter = [64, 128, 256, 512, 512]
        self.class_nb = 13
        ...
        #     nn.Linear(192, 96),  # Updated input size after pooling
        #     nn.ReLU(),
        #     nn.Linear(96, 48),
        #     nn.ReLU(),
        #     nn.Linear(48, 3),     # Output two weights for task 1 and task 2
        #     # nn.ReLU(),
        #     # nn.Sigmoid()
        #     nn.Softmax(dim=-1)    # Softmax to ensure weights sum to 1
        # )
        self.weight_network = nn.Sequential(
            nn.Linear(192, 3),  # Updated input size after pooling
            nn.Softmax(dim=-1)    # Softmax to ensure weights sum to 1
        )
        
        # self.weights = torch.nn.Parameter(torch.rand(3) * 0.8 + 0.2)
        ...
        #     # torch.nn.Softplus(),
        #     # nn.Softmax(dim=-1),
        #     nn.Sigmoid(),  # Transform raw weights to [0, 1]
        #     LambdaLayer(lambda x: x * self.scale_factor + self.lower_bound),  # Scale and shift
        #     nn.Softplus()  # Apply Softplus to ensure positivity
        # )
    ...
    def forward(self, x):
        ...
        combined_features = torch.cat(
            [atten_decoder[i][-1][-1] for i in range(3)], dim=1
        )
        pooled_features = F.adaptive_avg_pool2d(combined_features, (1, 1)).squeeze(-1).squeeze(-1)
        global_features = pooled_features.mean(dim=0)
        weights = self.weight_network(global_features)
        weights = torch.clamp(weights, min=0.2, max=0.8)
        # define task prediction layers
        t1_pred = F.log_softmax(self.pred_task1(atten_decoder[0][-1][-1]), dim=1)
        t2_pred = self.pred_task2(atten_decoder[1][-1][-1])
        t3_pred = self.pred_task3(atten_decoder[2][-1][-1])
        ...
        return (
            [t1_pred, t2_pred, t3_pred, weights],
            (
                atten_decoder[0][-1][-1],
                atten_decoder[1][-1][-1],
                atten_decoder[2][-1][-1],
...
class FairSegNetMtan(nn.Module):
    def __init__(self):
        super().__init__()
        self.segnet = _FairSegNet()
    ...
    def rounter_parameters(self) -> Iterator[nn.parameter.Parameter]:
        return (p for n, p in self.segnet.named_parameters() if "weight_network" in n)
    
    def model_parameters(self) -> Iterator[nn.parameter.Parameter]:
        return (p for n, p in self.segnet.named_parameters() if "weight_network" not in n)
